  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const explorerResponse = await fetch('http://192.168.1.8:3000/admin/explorer');
        const explorersData = await explorerResponse.json();
        setExplorersCount(explorersData.length);

        const businessResponse = await fetch('http://192.168.1.8:3000/admin/business');
        const businessData = await businessResponse.json();
        setBusinessOwnersCount(businessData.length);

        const postsResponse = await fetch('http://192.168.1.8:3000/posts/allposts');
        const postsData = await postsResponse.json();
        setPostsCount(postsData.length); 

        const eventsResponse = await fetch('http://192.168.1.8:3000/events/getAll');
