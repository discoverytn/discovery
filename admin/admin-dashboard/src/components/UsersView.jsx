...
function UsersView() {
  ...
  useEffect(() => {
    fetch("http://192.168.1.8:3000/admin/explorer")
  ...
  useEffect(() => {
    fetch("http://192.168.1.8:3000/admin/business")
      .then((response) => response.json())
      .then((data) => {
        if (Array.isArray(data)) {
  ...
  const deleteExplorer = async (idexplorer) => {
    try {
      const response = await fetch(
        `http://192.168.1.8:3000/admin/delete/explorer/${idexplorer}`,
        {
          method: "DELETE",
        }
      );
  ...
  const deleteBusinessOwner = async (idbusiness) => {
    try {
      const response = await fetch(
        `http://192.168.1.8:3000/admin/delete/business/${idbusiness}`,
        {
          method: "DELETE",
        }
      );
