...
const BusinessProfileScreen = () => {
  ...
  useEffect(() => {
    const fetchBusinessData = async () => {
      try {
        const response = await axios.get(`http://192.168.1.15:3000/business/${business.id}`);
  ...
  useEffect(() => {
    const fetchBusinessPosts = async () => {
      try {
        const response = await axios.get(`http://192.168.1.15:3000/business/${business.id}/posts`);
        if (response.status === 200) 
    ...
    const fetchBusinessEvents = async () => {
      try {
        const response = await axios.get(`http://192.168.1.15:3000/business/${business.id}/events`);
        if (response.status === 200) {
