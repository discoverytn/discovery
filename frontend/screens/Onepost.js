...
const OnepostScreen = ({ route }) => {
  ...
  const fetchPostDetails = async (postId) => {
    try {
      const response = await axios.get(`http://192.168.58.72:3000/posts/onepost/${postId}`);
  ...
  const checkIfPostFavorited = async (postId) => {
    try {
      ...
      const response = await axios.get(`http://192.168.58.72:3000/explorer/${idexplorer}/favourites/${postId}/check`);
  ...
  const checkIfPostTraveled = async (postId) => {
    try {
      const idexplorer = explorer.idexplorer;
      const response = await axios.get(`http://192.168.58.72:3000/explorer/${idexplorer}/traveled/${postId}/check`);
...
const addToFavorites = async () => {
  try {
    const idexplorer = explorer.idexplorer;
    const response = await axios.post(`http://192.168.58.72:3000/explorer/${idexplorer}/favourites/${postId}/addOrRemove`, {
    ...
    if (response.data.message === "Post added to favorites") {
      ...
      await axios.post('http://192.168.58.72:3000/notifications/create', {
...
const addToTraveled = async () => {
  try {
    const idexplorer = explorer.idexplorer;
    const response = await axios.post(`http://192.168.58.72:3000/explorer/${idexplorer}/traveled/${postId}/addOrRemove`, {
...
const fetchComments = async (postId) => {
  try {
    const response = await axios.get(`http://192.168.58.72:3000/comments/post/${postId}`);
...
const handleAddComment = async () => {
  ...
  try {
    ...
    const response = await axios.post('http://192.168.58.72:3000/comments/create', commentData);
...
const handleEditComment = async (commentId, newContent) => {
  try {
    await axios.put(`http://192.168.58.72:3000/comments/${commentId}`, { content: newContent });
...
const handleDeleteComment = async (commentId) => {
  try {
    await axios.delete(`http://192.168.58.72:3000/comments/${commentId}`);
