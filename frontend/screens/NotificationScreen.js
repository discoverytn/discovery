...
const NotificationScreen = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`http://192.168.142.72:3000/notifications/user/${userId}?userType=${userType}`);
  ...
  const markAsRead = async (notificationId) => {
    try {
      await axios.put(`http://192.168.142.72:3000/notifications/${notificationId}/read`);
