...
const ExplorerProfile = () => {
  ...
  useEffect(() => {
    const fetchExplorerData = async () => {
      try {
        const response = await axios.get(`http://192.168.58.72:3000/explorer/${explorer.id}`);
  ...
  useEffect(() => {
    const fetchExplorerPosts = async () => {
      try {
        const response = await axios.get(`http://192.168.58.72:3000/explorer/${explorer.id}/posts`);
        if (response.status === 200) {
  ...
  const deleteExplorerPost = async (postId, token) => {
    try {
      const response = await fetch(`http://192.168.58.72:3000/posts/explorer/delete/${postId}`, {
        method: 'DELETE',
        headers: {
