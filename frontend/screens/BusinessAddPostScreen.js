...
const BusinessAddPostScreen = () => {
  ...
  const fetchBusinessDetails = async (businessId) => {
    try {
      ...
      const response = await axios.get(`http://192.168.11.67:3000/admin/business/${businessId}`);
  ...
  const Submit = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.11.67:3000/posts/business/add', payload);
