...
const Rating = ({ postId, onRate }) => {
  ...
  const fetchRatings = async () => {
    try {
      ...
      const userRatingResponse = await fetch('http://192.168.11.112:3000/ratings/user-rating', {
        method: 'POST',
        headers: {
      ...
      const averageRatingResponse = await fetch(`http://192.168.11.112:3000/ratings/average-rating/${postId}`);
  ...
  const handleRate = async (rating) => {
    try {
      const response = await fetch('http://192.168.11.112:3000/ratings/rate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
