...
const Rating = ({ postId, onRate }) => {
  ...
  const fetchRatings = async () => {
    try {
      ...
      const userRatingResponse = await fetch('http://192.168.1.8:3000/ratings/user-rating', {
        method: 'POST',
        headers: {
      ...
      const averageRatingResponse = await fetch(`http://192.168.1.8:3000/ratings/average-rating/${postId}`);
