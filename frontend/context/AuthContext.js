...
const AuthProvider = ({ children }) => {
  ...
  const loginAction = async (data) => {
    try {
      const response = await axios.post("http://192.168.26.72:3000/auth/login", data);
  ...
  const signupAction = async (data) => {
    ...
    try {
      const endpoint =
        data.role === "explorer"
          ? "http://192.168.26.72:3000/auth/register/explorer"
          : "http://192.168.26.72:3000/auth/register/business";
